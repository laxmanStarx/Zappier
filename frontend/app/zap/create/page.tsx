    ...
    return <div>
        ...
        <div className="w-full min-h-screen bg-slate-200 flex flex-col justify-center">
            <div className="flex justify-center w-full" >
                ...
                <ZapCell onClick={() => { setSelectedModalIndex(1) }}
            ...
            <div className="w-full pt-2 pb-2">
                {selectedActions.map((action, index) => <div className="pt-2 flex justify-center">
                    <ZapCell onClick={() => { setSelectedModalIndex(action.index) }} name={action.availableActionId ? action.availableActionName : "Action"} index={action.index} /></div>)}
...
function Modal({ index, onSelect , availableItems}: { index: number, onSelect: (props: null | { name: string; id: string, metadata:any }) => void, availableItems: {id: string, name: string, image: string; }[] }) {
    ...
    return <div  className="  fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full bg-slate-100 bg-opacity-75 flex">
...
function EmailSelector({setMetadata}:{
    ...
    return <div>
        <Input label={"To"} type={"text"} placeholder="Email Sender" onChange={(e)=>setEmail(e.target.value)}></Input>
        <Input label={"Body"} type={"text"} placeholder="Email Sender" onChange={(e)=>setBody(e.target.value)}></Input>
        <div className="pt-2">
        <PrimaryButton onClick={()=>{
            setMetadata({
                email,
...
function SolanaSelector({setMetadata}:{
    ...
    return<div>
        <Input label={"To"} type={"text"} placeholder="Email Sender" onChange={(e)=>setAddress(e.target.value)}></Input>
        <Input label={"Amount"} type={"text"} placeholder="Email Sender" onChange={(e)=>setAmount(e.target.value)}></Input>
        <PrimaryButton onClick={()=>{
            setMetadata({
                amount,
                address
